...
import { normalizeReportRequirements } from '@/lib/requirements';
...
import { X, Plus, Lightbulb, Funnel } from '@phosphor-icons/react';
...
export function ReportRequirementsForm({
  ...
  const [slicerCount, setSlicerCount] = useState(
    initialRequirements?.slicerCount || 2
  );
  ...
  const handleSubmit = () => {
    ...
    const requirements: ReportRequirements = normalizeReportRequirements({
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <Label>Number of Slicers</Label>
            </div>
            <div className="flex items-center gap-4">
              <Slider
                value={[slicerCount]}
                onValueChange={(value) => setSlicerCount(value[0])}
                min={1}
                max={6}
                step={1}
                className="flex-1"
              />
              <div className="flex items-center justify-center min-w-[3rem] h-10 px-3 rounded-md bg-muted font-semibold text-sm">
                {slicerCount}
              </div>
            </div>
            <p className="text-xs text-muted-foreground">
              Slicers allow users to filter data in the report. Recommended: 2-4 slicers.
            </p>
          </div>
